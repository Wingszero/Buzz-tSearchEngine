...
def real_main():
    ...
    elif action == "install-java":
        (master_nodes, slave_nodes) = get_existing_cluster(conn, opts, cluster_name)
        print("Installing java 8...")
        commands = ['sudo yum -y install java-1.8.0', 'sudo yum -y install java-1.8.0-openjdk-devel', 'sudo yum -y remove java-1.7.0', 'sudo ln -fs /usr/lib/jvm/java-1.8.0 /usr/lib/jvm/java-1.7.0']
        master = get_dns_name(master_nodes[0], opts.private_ips)
    ...
    elif action == "move-spark-workdir":
        (master_nodes, slave_nodes) = get_existing_cluster(conn, opts, cluster_name)
        print("removing workdir")
        commands = ['sudo mkdir -p /vol0/work', 'sudo rm -rf /root/spark/work', 'sudo ln -fs /vol0/work /root/spark/work', 'sudo mkdir -p /vol0/spark', 'sudo rm -rf /mnt/spark', 'sudo ln -fs /vol0/spark /mnt/spark']
        for slave in slave_nodes:
            slave_address = get_dns_name(slave, opts.private_ips)
            print(slave_address)
            for command in commands:
